    ...
    <div className="flex flex-col w-full pt-32 bg-[#fafafa]">
      <section className="container mx-auto px-4 md:px-10 max-w-7xl pb-24">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div>
            ...
            <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-[clamp(2rem,4vw,3.5rem)] font-heading font-black text-[#0D1117] leading-tight mb-6">
            ...
            <div className="space-y-8">
              {[
                ...
                <motion.div key={item.title} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + (i * 0.1) }} className="flex gap-5">
                  ...
                  <div>
                    <h3 className="font-heading font-bold text-[#0D1117] text-[15px] mb-1">{item.title}</h3>
          ...
          <motion.div 
            ...
            className="bg-white rounded-[2rem] p-8 sm:p-10 shadow-[0_8px_40px_rgba(0,0,0,0.04)] border border-gray-100"
          >
            <h3 className="font-heading font-bold text-[#0D1117] text-xl mb-6">Send us a message</h3>
      ...
      <div className="h-96 w-full bg-gray-200 relative">
        <div className="absolute inset-0 flex items-center justify-center">
          <p className="text-gray-400 font-medium flex items-center gap-2"><MapPin size={18} /> Interactive Map Embed Here</p>
        </div>
      </div>
    </div>
  );
